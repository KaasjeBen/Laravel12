...
<form method="POST" action="{{ route('tasks.update', $task->id) }}" class="space-y-4">
    ...
    <div>
        ...
        <input type="date" name="begindate" class="w-full border p-2" value="{{ old('begindate', $task->begindate?->toDateString()) }}">
    </div>
    <div>
        <label class="block mb-1">Einddatum</label>
        <input type="date" name="enddate" class="w-full border p-2" value="{{ old('enddate', $task->enddate?->toDateString()) }}">
    </div>
    <div>
